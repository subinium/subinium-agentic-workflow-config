...
| **`memory-curate`** | `/memory-curate` | `~/.claude/projects/*/memory/` 감사 — 고아 파일, 중복, 분류 위반, `MEMORY.md` 비대. 기본 읽기 전용; `--prune`로 적용. |
...
| **`audit-migration`** | `/audit-migration` | SQL/Prisma/Supabase/sqlx/Drizzle 마이그레이션 감사 — 데이터 유실, 락 경합, 누락된 인덱스, 롤백 안전성. `migration-reviewer` 에이전트 래퍼. |
| **`audit-i18n-nfc`** | `/audit-i18n-nfc` | CJK 문자열 처리 NFC/NFD 정규화 불일치 감사 — 파일명, URL, 첨부 경로. `i18n-nfc-auditor` 에이전트 래퍼. |
...
| **`role-routing.md`** | 겹치는 스킬/에이전트 결정 트리 — 계획 체인(`prd-extract` → `architect`), 리뷰 체인(`/code-review` → `reviewer` → 플러그인 `/review`), 리서치 체인(스페셜리스트 우선). 중복 체이닝 방지. |
...
| **`guard-bash-secrets.sh`** | `PreToolUse` (Bash) | Bash로 시크릿 우회 차단 — `cat .env`, `head ~/.ssh/id_rsa`, `source .env`, `grep KEY .env.production`, `cp .env /tmp/`. 파일 경로 deny가 못 잡는 구멍 차단. |
...
```
요청 → settings.json deny → guard-sensitive-files.sh / guard-bash-secrets.sh / block-destructive-git.sh → CLAUDE.md 규칙 → 실행
```

| 보호 대상 | settings.json deny (정적) | 파일 경로 훅 | bash 훅 | CLAUDE.md (행동) |
|----------|---------------------------|-------------|---------|-----------------|
| `.env` / `.envrc` / secrets | Read+Write 차단 | `guard-sensitive-files.sh` | `guard-bash-secrets.sh` (cat / source / grep / cp) | "크레덴셜 커밋 금지" |
| 개인키 (`.pem`, `.key`, `id_*`) | Read 차단 | `guard-sensitive-files.sh` | `guard-bash-secrets.sh` | — |
| `.ssh/`, `.aws/`, `.kube/`, `.npmrc`, `.netrc` | Read+Write 차단 | `guard-sensitive-files.sh` | `guard-bash-secrets.sh` | — |
